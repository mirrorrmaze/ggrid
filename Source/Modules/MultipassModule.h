...
namespace GGrid
{
    ...
    // summing the bands back together: each band's post-split, post-Mix content is handed out
    ...
    private:
        ...
        std::array<std::atomic<float>*, kNumMultipassBands> mixParams {};

        // Post-Mix, per-band content -- what getOutputBusBuffer hands out. Sized/cleared each
        ...
        juce::AudioBuffer<float> remaining, lowScratch, dryBuffer;
